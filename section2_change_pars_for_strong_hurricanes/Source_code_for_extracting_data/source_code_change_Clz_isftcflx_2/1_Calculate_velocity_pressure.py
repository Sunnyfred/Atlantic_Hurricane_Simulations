...
for gk in range(len(gridsize)):
    ...
    for Hurricane in Hurricaneall:
        ...
        for Dir in Dirall:
            ...
            Dir_local = mainpath+Hurricane+ '/' +gridsize[gk]+ '/' +'WRFONLY_NoTurb_'+gridsize[gk]+Dir
            print(Dir_local)
            ...
            for ncfile in ncfiles:  
     	        ...
     	        pickle.dump( WSPD, open( outputpath+Hurricane+'_wspd_par'+str(count2+1)+'_'+gridsize[gk]+'.p', "wb" ) )
     	        pickle.dump( PRESSURE, open( outputpath+Hurricane+'_pressure_par'+str(count2+1)+'_'+gridsize[gk]+'.p', "wb" ) )       
