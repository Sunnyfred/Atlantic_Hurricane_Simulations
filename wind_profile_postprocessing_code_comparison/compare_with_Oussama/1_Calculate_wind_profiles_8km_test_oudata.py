...
mainpath = '/project/momen/oromdhani/WRF_3/Hurricanes/'
...
gridsize = ['8km']
swansize = [ 'swgr8p0']
prefix = 'Maria_1Nest_96Hours_MainGrid8km_NoTurb_vert42_cLh1p0_'
Dirall = ['cLz0p0001_cLzland1p0_cfl2p0_isftcflx2',
       'cLz1p0_cLzland1p0_cfl2p0_isftcflx2',
       'cLz100p0_cLzland1p0_cfl2p0_isftcflx2']
#Dirall = ['cLz1p0_cLzland1p0_cfl2p0_isftcflx2']
...
ncfiles = []
min_press_factor=0.995
max_press_factor=1.0

for i in range(len(avg_day)):
    for j in range(len(wrf_af)):
        #if i==0 and j==0:
        #    continue
        #else:
        #    ncfiles.append(wrf_pre+years_month[0]+avg_day[i]+wrf_af[j])
        ncfiles.append(wrf_pre+years_month[0]+avg_day[i]+wrf_af[j])
    ...
    for gk in range(len(gridsize)):
        ...
        for Hurricane in Hurricaneall:
            ...
            for Dir in Dirall:
                ...
                Dir_local = mainpath+Hurricane+ '/' +gridsize[gk]+ '/' +prefix+Dir
                print(Dir_local)
            ...
            with open(outputpath+Hurricane+'_wind_profiles_'+gridsize[gk]+'_oudata_'+str(count0)+'.csv', 'w') as csvfile:
