        ...
        for Hurricane in Hurricaneall:
            ...
            for Dir in Dirall:
                ...
                for ncfile in ncfiles:  
         	        ...
         	        max_wind_lat.append(latitudes[max_wind_idx[0]])
         	        max_wind_long.append(longitudes[max_wind_idx[1]])
         	        print(max_wind_lat, max_wind_long, eye_lat[0], eye_long[0])                     
         	        tmp = sin((radians(eye_lat[0]) - radians(max_wind_lat[0])) / 2)**2 + \
                            cos(radians(max_wind_lat[0])) * cos(radians(eye_lat[0])) *\
                                sin( (radians(eye_long[0]) - radians(max_wind_long[0])) / 2)**2
         	        eye_wall_distance = 2 * R * np.arcsin(sqrt(tmp))                        
         	        print('eye_wall_distance is: '+ str(eye_wall_distance))
         	        ...
         	        for lat_i in latitudes:
                         ...
                         for lon_i in longitudes:
                             ...
                             #print(lat_i, lon_i, eye_lat[0], eye_long[0])                             
                             ...
                             #if (float(distance) >= min_press_factor * float(eye_wall_distance)) \
                             #    and (float(distance) <= max_press_factor * float(eye_wall_distance)):
                             if (float(distance) >= float(eye_wall_distance-16)) \
                                 and (float(distance) <= float(eye_wall_distance+16)):  # 16 means 2 times of grid resolution
